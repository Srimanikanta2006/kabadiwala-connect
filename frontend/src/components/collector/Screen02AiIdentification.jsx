...
export default function Screen02AiIdentification({
  ...
  const baseRate = unit === 'piece'
    ...
    : (materialId === 'mat_pcb_high' ? 240 : (materialId === 'mat_cables_copper' ? 380 : (materialId === 'mat_batteries_li_ion' ? 185 : (materialId === 'mat_batteries_lead' ? 88 : (materialId === 'mat_motors_magnets' ? 145 : 105)))));
  ...
  <nav className="fixed bottom-0 left-0 w-full z-50 flex md:hidden justify-around items-center px-2 py-2 bg-surface border-t border-outline-variant shadow-md rounded-t-xl">
    ...
    <button
      ...
      className={`flex flex-col items-center justify-center cursor-pointer transition-colors ${
        activeScreen === 'my_lots' || activeScreen === 'ai_scan' ? 'bg-primary-container text-on-primary-container rounded-full px-4 py-1 scale-90' : 'p-2 text-on-surface-variant'
      }`}
    >
      <span className={`material-symbols-outlined ${activeScreen === 'my_lots' || activeScreen === 'ai_scan' ? 'filled' : ''}`}>inventory_2</span>

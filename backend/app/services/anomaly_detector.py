...
Enforces:
1. Physical weight-density bounds per material type.
2. Statistical rate outlier checks (IQR / Z-score filters).
3. Image perceptual hash (pHash/dHash) duplicate detection.
"""

from typing import Dict, Any, List, Optional

# Plausible single-lot weight boundaries (kg)
...
def check_weight_plausibility(material_id: str, weight_kg: float) -> Optional[Dict[str, Any]]:
    """Checks if entered weight falls within plausible single-collection bounds."""
...
def hamming_distance(hash1: str, hash2: str) -> int:
    """Calculates Hamming distance between two hex perceptual hash strings."""
    try:
        val1 = int(hash1, 16)
        val2 = int(hash2, 16)
        xor_val = val1 ^ val2
...
def check_duplicate_image(new_hash: str, recent_hashes: List[str], threshold: int = 4) -> bool:
    """Flags if an identical or near-duplicate scrap photo was recently uploaded."""
    if not new_hash:
        return False
